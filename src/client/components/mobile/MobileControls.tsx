...
export function MobileControls() {
  ...
  const onLookPointerDown = (e: React.PointerEvent) => {
    ...
    if (target.closest && (target.closest('.virtual-joystick') || target.closest('.touch-button'))) return;
  ...
  const onLookPointerUp = (e: React.PointerEvent) => {
    if (e.pointerId !== lookPointerId.current) return;
    try { (e.target as Element).releasePointerCapture?.(e.pointerId as any); } catch {}
  ...
  return (
    <div className="mobile-controls" style={{ pointerEvents: 'none' }}>
      <div className="virtual-joystick-wrapper" style={{ pointerEvents: 'auto' }}>
      ...
      <div className="mobile-buttons" style={{ pointerEvents: 'auto' }}>
        <TouchButton label="발사" onPress={() => inputManager.setTouchFire(true)} onRelease={() => inputManager.setTouchFire(false)} />
        <TouchButton label="점프" onPress={() => inputManager.setTouchJump(true)} onRelease={() => inputManager.setTouchJump(false)} />
        <TouchButton label="슬라이드" onPress={() => inputManager.setTouchCrouch(true)} onRelease={() => inputManager.setTouchCrouch(false)} />
        <TouchButton label="재장전" onPress={() => inputManager.setTouchReload(true)} onRelease={() => inputManager.setTouchReload(false)} />
      </div>
    </div>
  );
}
